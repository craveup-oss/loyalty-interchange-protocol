...
<CardGroup cols={2}>
  ...
  <Card title="llms.txt index" icon="file-lines" href="https://github.com/craveup-oss/loyalty-interchange-protocol/blob/main/llms.txt">
...
<Steps>
  <Step title="Point your agent at llms.txt">
    Load the checked-in [`llms.txt`](https://github.com/craveup-oss/loyalty-interchange-protocol/blob/main/llms.txt) so the agent reads the normative spec before changing behavior.
  ...
  <Step title="Connect the MCP server">
    Enable the repo-root [`mcp.json`](https://github.com/craveup-oss/loyalty-interchange-protocol/blob/main/mcp.json), or run the published server standalone.
...
The repo root includes [`llms.txt`](https://github.com/craveup-oss/loyalty-interchange-protocol/blob/main/llms.txt), a compact index of the normative spec, OpenAPI contract, SDK entry points, CLI commands, and the files agents should read before changing behavior. Point your agent at it first:
...
LIP ships installable [Agent Skills](https://agentskills.io/) in the [`skills/`](https://github.com/craveup-oss/loyalty-interchange-protocol/tree/main/skills) directory. They give coding agents specialized knowledge for the checkout lifecycle, webhooks, the BFF pattern, SDK usage, and conformance.
...
Works with Cursor, Claude Code, Codex, Windsurf, GitHub Copilot, and other agents that support the skills format. See the [Skills README](https://github.com/craveup-oss/loyalty-interchange-protocol/blob/main/skills/README.md) for the full catalog.
...
**Cursor:** enable the repo-root [`mcp.json`](https://github.com/craveup-oss/loyalty-interchange-protocol/blob/main/mcp.json) (Settings → MCP).
...
| --- | --- | --- |
| OpenAPI 3.1 binding | [`spec/openapi.yaml`](https://github.com/craveup-oss/loyalty-interchange-protocol/blob/main/spec/openapi.yaml) | HTTP paths, auth, request/response shapes |
| JSON Schemas | [`spec/schemas/`](https://github.com/craveup-oss/loyalty-interchange-protocol/tree/main/spec/schemas) | Payload validation and code generation |
| Lifecycle rules | [`spec/lifecycle.md`](https://github.com/craveup-oss/loyalty-interchange-protocol/blob/main/spec/lifecycle.md) | Reserve → capture → reverse, refunds, idempotency |
| Foodservice profile | [`spec/profiles/foodservice.md`](https://github.com/craveup-oss/loyalty-interchange-protocol/blob/main/spec/profiles/foodservice.md) | Order lines, modifiers, tenders, earning rules |
| Webhook profile | [`spec/webhooks.md`](https://github.com/craveup-oss/loyalty-interchange-protocol/blob/main/spec/webhooks.md) | Signature verification and deduplication |
| Full lifecycle example | [`examples/typescript/full-lifecycle.ts`](https://github.com/craveup-oss/loyalty-interchange-protocol/blob/main/examples/typescript/full-lifecycle.ts) | End-to-end SDK usage |
...
<Steps>
  <Step title="llms.txt">
    Start with the [index](https://github.com/craveup-oss/loyalty-interchange-protocol/blob/main/llms.txt) to orient on what exists.
  </Step>
  <Step title="Lifecycle rules">
    Read [`spec/lifecycle.md`](https://github.com/craveup-oss/loyalty-interchange-protocol/blob/main/spec/lifecycle.md) for reserve → capture → reverse and idempotency.
  </Step>
  <Step title="Full lifecycle example">
    Study [`examples/typescript/full-lifecycle.ts`](https://github.com/craveup-oss/loyalty-interchange-protocol/blob/main/examples/typescript/full-lifecycle.ts) for end-to-end SDK usage.
  </Step>
