...
<Note>
`lip-dev-key` is the source-runtime default. The loopback-only Compose evaluation
profile uses `lip-local-compose-key-change-me` because network-bound server
processes reject the short development key. Replace either key outside a local
