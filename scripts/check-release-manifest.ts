...
const PUBLISHED_PACKAGE_NAME_SET = new Set<string>(PUBLISHED_PACKAGE_NAMES);
...
export function validateLipReleaseManifest(value: unknown): LipReleaseManifestV1 {
  ...
  if (source) {
    ...
    expectExact(source.repository, "craveup-oss/loyalty-interchange-protocol", "source.repository", errors);
  ...
  if (protocol) {
    expectOnlyKeys(protocol, ["version", "profile", "openapiSha256"], "protocol", errors);
  ...
  if (packages) {
    if (packages.length !== PUBLISHED_PACKAGE_NAMES.length) {
      errors.push(`packages must contain exactly ${PUBLISHED_PACKAGE_NAMES.length} published packages`);
    }
    const seen = new Set<string>();
    packages.forEach((entry, index) => {
      const item = objectAt(entry, `packages[${index}]`, errors);
      if (!item) return;
      expectOnlyKeys(item, ["name", "version", "integrity"], `packages[${index}]`, errors);
      ...
      if (name) {
        ...
        if (!PUBLISHED_PACKAGE_NAME_SET.has(name)) {
          errors.push(`packages[${index}].name must be one of the published package names`);
        }
    ...
    for (const expected of PUBLISHED_PACKAGE_NAMES) {
      if (!seen.has(expected)) errors.push(`packages must include ${expected}`);
    }
  }

  const image = objectAt(root.image, "image", errors);
  if (image) {
    expectOnlyKeys(image, ["reference", "digest", "provenanceUrl"], "image", errors);
  ...
  if (database) {
    expectOnlyKeys(database, ["migrationSetSha256", "connectionMode"], "database", errors);
  ...
  if (dependencies) {
    expectOnlyKeys(
      dependencies,
      ["lockfileSha256", "auditReportSha256", "sbomSha256", "riskRegisterSha256", "toolchain", "findings"],
      "dependencies",
      errors
    );
    ...
    if (toolchain) {
      expectOnlyKeys(toolchain, ["node", "npm"], "dependencies.toolchain", errors);
    ...
    if (findings) {
      expectOnlyKeys(
        findings,
        ["moderate", "high", "critical", "unapprovedHigh", "unapprovedCritical"],
  ...
  if (verification) {
    expectOnlyKeys(verification, ["runUrl", "conformanceReportSha256"], "verification", errors);
...
function expectNpmSri(value: unknown, path: string, errors: string[]): void {
  const text = stringAt(value, path, errors);
  if (!text) return;
  if (!text.startsWith(NPM_SRI_PREFIX)) {
    errors.push(`${path} has invalid format`);
    return;
  }

  const digest = text.slice(NPM_SRI_PREFIX.length);
  if (!BASE64.test(digest)) {
    errors.push(`${path} has invalid format`);
    return;
  }
  ...
  if (decoded.length !== 64 || decoded.toString("base64") !== digest) {
    errors.push(`${path} must contain a canonical SHA-512 digest`);
  }
}

function expectOnlyKeys(value: JsonRecord, allowed: readonly string[], path: string, errors: string[]): void {
